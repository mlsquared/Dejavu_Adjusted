...
CONNECTIVITY_MASK = getenv_int("CONNECTIVITY_MASK", 2)

# Sampling rate, related to the Nyquist conditions, which affects
# the range frequencies we can detect.
DEFAULT_FS = getenv_int("DEFAULT_FS", 44100)

# Size of the FFT window, affects frequency granularity
DEFAULT_WINDOW_SIZE = getenv_int("DEFAULT_WINDOW_SIZE", 4096)

# Ratio by which each sequential window overlaps the last and the
# next window. Higher overlap will allow a higher granularity of offset
# matching, but potentially more fingerprints.
DEFAULT_OVERLAP_RATIO = getenv_float("DEFAULT_OVERLAP_RATIO", 0.5)
...
DEFAULT_FAN_VALUE = getenv_int("DEFAULT_FAN_VALUE", 5)  # 15 was the original value.

# Minimum amplitude in spectrogram in order to be considered a peak.
# This can be raised to reduce number of fingerprints, but can negatively
# affect accuracy.
DEFAULT_AMP_MIN = getenv_int("DEFAULT_AMP_MIN", 10)

# Number of cells around an amplitude peak in the spectrogram in order
# for Dejavu to consider it a spectral peak. Higher values mean less
# fingerprints and faster matching, but can potentially affect accuracy.
PEAK_NEIGHBORHOOD_SIZE = getenv_int("PEAK_NEIGHBORHOOD_SIZE", 10)  # 20 was the original value.

# Thresholds on how close or far fingerprints can be in time in order
# to be paired as a fingerprint. If your max is too low, higher values of
# DEFAULT_FAN_VALUE may not perform as expected.
MIN_HASH_TIME_DELTA = getenv_int("MIN_HASH_TIME_DELTA", 0)
MAX_HASH_TIME_DELTA = getenv_int("MAX_HASH_TIME_DELTA", 200)
...
FINGERPRINT_REDUCTION = getenv_int("FINGERPRINT_REDUCTION", 20)
